...
class NewsController extends Controller
{
	...
	public function show($title_slug) {
		$news = $this->news->where('title_slug',$title_slug)->get(['id','title','content','posted_at','created_by','view_mode'])->first();
		$id = $news['id'];
		$user = Auth::User();
		...
		if ($news->view_mode == 'association') $title = Lang::get('news/backend.association');
		if ($news->view_mode == 'event') $title = Lang::get('news/backend.event');
		return view('news.show',compact(['news','news_recent','news_hot','categories','id','comment','user','title']));
	}
	...
	public function getNewsFromCategory($category_slug=null) {
		...
		return view('news.index',compact(['news','news_recent','news_hot','categories','news_news','title']));
	}
	...
	public function getNewsAssociation() {
		...
		$title = Lang::get('general.association_news');
		return view('news.index',compact(['news','news_recent','news_hot','categories','news_news','title']));
	}

	public function getHotNews() {
		$news =  $this->news->getHotNews();
		$news_hot = $this->news->getHotNews();
		$news_recent = $this->news->getRecentNews();
		$categories = $this->category->getCategoryMenu();
		$news_news = $this->news->getNewsByCategory('Tin tức');

		$title = Lang::get('general.hot_news');
		return view('news.index',compact(['news','news_recent','news_hot','categories','news_news','title']));
	}
	...
	public function getBranchNews() {
		...
		$title = Lang::get('general.branch_news');
		return view('news.index',compact(['news','news_recent','news_hot','categories','news_news','title']));
	}

	public function getEvents() {
		$news =  $this->news->getEvents();
		$news_hot = $this->news->getHotNews();
		$news_recent = $this->news->getRecentNews();
		$categories = $this->category->getCategoryMenu();
		$news_news = $this->news->getNewsByCategory('Tin tức');

		$title = Lang::get('general.event');
		return view('news.index',compact(['news','news_recent','news_hot','categories','news_news','title']));
	}
